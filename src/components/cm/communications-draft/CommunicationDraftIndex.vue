	...
	<div class="row">
        <div class="col pAgeEmail--title">
            <ul class="d-flex list-unstyled align-items-center mh-45 mb-2">
                <li class="col col-auto me-4"><h2 class="ps-1">Draft</h2></li>
            ...
            <div class="min-vh-75 bg-mid-trans tblWrap f85 mb-3 pb-3">
            	<table class="tbl tblEmail--Wrap draft table">
                    <thead>
                        <tr>
                            <th>
                                <input class="chk__p form-check-input" name="cA" type="checkbox" @change="checkParentDraft" v-model="allcheckeddraft">                          
                            </th> 
                            <th colspan="2">  
                    ...
                    <tbody>
                        ...
                        <template v-if="drafts.length > 0">
                            <template v-for="draft in drafts" :key="draft.id">
                                <tr @click="show(draft.id)">
                                    <td @click.stop><input name="cs" class="form-check-input chk__c" type="checkbox" :value="draft.id" v-model="formData.selectedDraft" @change='draftCheck()'></td>
                                    <td>
                        ...
                        <template v-if="noData">                                
                            <tr class="nodata pr">
                                <td colspan="6"><span class="nodata">Draft is Empty </span>
                                </td>
                            </tr>
                        </template>
                    </tbody>
	...
	export default{
		...
		setup(){
           ...
           const show = (id) => {
            ...
            const showPage = (n) => {
                if(n == 1){
                    const id = draftLinks.value.next.match(/([^\/]*)\/*$/)[1].split('=');
                    router.push({name: 'communications-draftpage.show', params: { id: id[id.length-1] }});
                }else{
                    const id = draftLinks.value.prev.match(/([^\/]*)\/*$/)[1].split('=');
                    router.push({name: 'communications-draftpage.show', params: { id: id[id.length-1] }});
                }
            }
