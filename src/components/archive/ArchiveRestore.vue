    ...
    <form v-on:submit.prevent="saveEmployee">
        ...
        <div class="form-row">
            <div class="col mt-3 text-end">
                <router-link :to="{name: 'archives.index'}" class="btn btn-secondary me-1">Cancel</router-link>
...
export default{
    ...
    setup (props){
        ...
        const {archive, getArchiveEmployee,restoreArchived, errors} = useArchive()
        ...
        const loadarchiveInfo = async() =>{
            getArchiveEmployee(props.id).then(res => {

                form.employee_fname = archive.value.employee_fname;
                form.employee_mname = archive.value.employee_mname;
                form.employee_lname = archive.value.employee_lname;
                form.employee_extname = archive.value.employee_extname;
                form.emergency_contactperson = archive.value.emergency_contactperson;
                form.emergency_contactnumber = archive.value.emergency_contactnumber;

       
                if(archive.value.archiveemployments){
                    for (var i = 0; i < archive.value.archiveemployments.length; i++) {
                        divs.push({
                            id: add.value,
                        })
                        ...
                        form.employments.push({
                            ...
                            'employee_id' : '',
                            'employmentstatus' : '',
                            'type_id' : archive.value.archiveemployments[i].type_id,
                            'office_id' : archive.value.archiveemployments[i].office_id,
                            'status_id' : archive.value.archiveemployments[i].status_id,
                            'position_id' : archive.value.archiveemployments[i].position_id,
                            'salarygradegroups_id' : archive.value.archiveemployments[i].salarygradegroups_id,
                            'salarygrades_id' : archive.value.archiveemployments[i].salarygrades_id,
                            'salarygrades_steps_id' : archive.value.archiveemployments[i].salarygrades_steps_id,
                            'startdate' : archive.value.archiveemployments[i].startdate,
                            'enddate' : archive.value.archiveemployments[i].enddate,
                            'enddatestatus' : archive.value.archiveemployments[i].enddatestatus,
                            'dateupdated' : '',
                            'new' : 1,
                        });

                        if(archive.value.archiveemployments[i].salarygradegroups_id){
                            z = i;
                            getSalaryGradeName(archive.value.archiveemployments[i].salarygradegroups_id).then(() => {
                                 salarygradenamesArr.value[z] = salarygradenames.value;
                            });
                            getSalaryGradeSteps(archive.value.archiveemployments[i].salarygrades_id).then(() => {
                                salarygradestepsArr.value[z] = salarygradesteps.value;  
                            });
                        }
        ...
        const saveEmployee = async () => {
            ...
            await restoreArchived(archive.value.employee.employee_id,{...form}).then(() => {
                if(!errors.value){
                    swal.fire({
                        toast: true,
                        position: 'top-end',
