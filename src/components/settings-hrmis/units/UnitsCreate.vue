<template> 
    <div class="row">
        <div class="col-md-12 p-title">
            <h2>New Unit</h2>
        </div>
    </div>

        <form @submit.prevent="saveUnit" class="bg-light tblWrap mb-3 py-3 px-3">
            <div class="row">
                <div class="col col-sm-8">
                    <div class="form-floating req">                
                        <input type="text" name="name" placeholder="enter name" id="name" class="form-control" :class="errors.name ? 'is-invalid' : ''" v-model="form.name">
                        <label for="name" class="form-label">Name / Description</label>                
                        <span v-if="errors.name" class="text-danger m-error">{{errors.name[0]}}</span> 
                    </div>
                </div>
                <div class="col col-sm-4">
                    <div class="form-floating">                
                        <input type="text" name="cg_code" placeholder="enter code" id="slug" class="form-control" v-model="form.slug">
                        <label for="slug" class="form-label">SLUG</label> 
                    </div>
                </div>
            </div>

        <div class="form-row">
            <div class="col mt-3 text-end">
                <router-link :to="{name: 'units.index'}" class="btn btn-secondary me-1">Cancel</router-link>
                <button type="submit" class="btn btn-blue"> Save</button>
            </div>
        </div>
        
    </form>


    
</template>

<script>
    import { reactive,inject} from "vue";

    import useUnits from "@/composables/hrmis/composables-settingsunits";
    import { useHead } from '@unhead/vue'


    export default {
        setup(){
            useHead({
                title: 'Settings - New HRMIS Unit | '+import.meta.env.VITE_BFAR_AGENCY
            })
            const form = reactive({
                'name': '',
                'cg_code': '',
            });

            const swal = inject('$swal')
            
            const { errors, storeUnit } = useUnits()

            const saveUnit = async () => {
                await storeUnit({ ...form }).then(() => {
                    if(!errors.value){
                        swal.fire({
                            toast: true,
                            position: 'top-end',
                            title: 'Unit Successfully Created',
                            showConfirmButton: false,            
                            icon: 'success',
                            width: '300',
                            padding: '.5em 1em',
                            timerProgressBar: true,
                            timer:1500,
                            customClass: {
                                container: 'swaltopright'
                            }
                        })
                    }
                });
            }
            
            return{
                form,
                errors,
                saveUnit
            }
        }
    }
</script>